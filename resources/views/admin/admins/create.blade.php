...
@section('title', 'إضافة حساب إداري')

@section('content')
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h5 class="card-title mb-0">إضافة حساب إداري</h5>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="account_type" id="account_type" class="form-select @error('account_type') is-invalid @enderror">
                            <option value="admin" {{ old('account_type') == 'admin' ? 'selected' : '' }}>مشرف</option>
                            <option value="data_entry_tourism_establishments" {{ old('account_type') == 'data_entry_tourism_establishments' ? 'selected' : '' }}>مدخل منشآت سياحية</option>
                            <option value="data_entry_attractions_routes" {{ old('account_type') == 'data_entry_attractions_routes' ? 'selected' : '' }}>مدخل مواقع الجذب والمسارات</option>
                        </select>
                    ...
                    <div class="row">
                        ...
                        @foreach($permissions as $key => $label)
                            <div class="col-md-3 mb-2">
                                <div class="form-check">
                                    <input type="checkbox" class="form-check-input permission-checkbox" id="perm_{{ $key }}" name="permissions[]" value="{{ $key }}" {{ is_array(old('permissions')) && in_array($key, old('permissions')) ? 'checked' : '' }}>
